...
### Upregulated in Tangle-bearing Neurons {.no-title}
...
shinyApp(
  ...
  ui = fluidPage(
    fluidRow(
      ...
      column(
        width = 4,
        radioButtons(
          paste0("path_up", 23), label = NULL,
          c(
            "Pathways Shared Between Datasets",
            "Pathways Shared Between Genes/Proteins"
          ),
          inline = TRUE
        ),
        style = "padding-top: 35px;"
  ...
  server = function (input, output, session) {
    observeEvent({
      ...
      net_data <- net_data[net_data$symbol %in% V(graph)$name, ]
      vertices <- data.frame(
      ...
      dup <- which(duplicated(edges))
      if (
        input[[paste0("path_up", 23)]] == "Pathways Shared Between Genes/Proteins"
      ) {
        edges <- edges[-dup, ]
      } else {
        edges <- edges[dup, ]
      ...
      if (
        ...
        graph_sub <- graph_sub[degree >= 2]
        graph <- induced_subgraph(
          graph, vids = unlist(
            neighborhood(graph, order = 1, nodes = graph_sub)
          )
        )
      }
      ...
      layout <- qgraph.layout.fruchtermanreingold(
        edges, vcount = vcount(graph), weights = E(graph)$weight,
        area = vcount(graph) / 4, repulse.rad = vcount(graph)
      )
      ...
      if (
        ...
        for (gene in vis2$nodes$id) {
          if (gene %in% names(keep_table_comb)) {
            ...
            keep_table_sub <- (((keep_table_sub + 2) ^ 4) / 4) + 16
            size <- c(size, keep_table_sub)
          } else {
            size <- c(size, 20)
          }
        ...
        for (gene in vis2$nodes$id) {
          if (gene %in% names(keep_table_comb)) {
            ...
            keep_table_sub <- (((keep_table_sub + 2) ^ 4) / 4) + 26
            size <- c(size, keep_table_sub)
          } else {
            size <- c(size, 30)
          }
      ...
      vis2$edges$width <- rep(5, times = nrow(vis2$edges))
      ...
      if (length(names(V(graph))) > 0) {
        ...
        f <- pmax((max(nchar(net_data$symbol)) - nchar(vis2$nodes$id)) / 2, 0)
        vis2$nodes$label <- sprintf(
          "%-*s%s%*s", f, "", vis2$nodes$id, ceiling(f), ""
        )
        ...
        vis2 <- visIgraphLayout(vis2, "layout.norm", layoutMatrix = layout)
        vis2 <- visOptions(
          vis2,
          highlightNearest = list(
            enabled = TRUE,
