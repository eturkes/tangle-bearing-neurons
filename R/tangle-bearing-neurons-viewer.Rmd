...
net_data_table_sub <- net_data_table_sub[
  names(net_data_table_sub) %in% vis_up$nodes$id
]
vis_up$nodes$size <- (net_data_table_sub + 5) ^ 2
vis_up$edges$width <- vis_up$edges$weight ^ 1.5

net_res <- res_deg[[i]][res_deg[[i]]$Gene %in% vis_up$nodes$id, ]
...
for (j in seq_along(title)) {
  net_data_sub <- net_data_up[net_data_up$symbol == vis_up$nodes$id[j], ]
...
net_data_table_sub <- net_data_table_sub[
  names(net_data_table_sub) %in% vis_dn$nodes$id
]
vis_dn$nodes$size <- (net_data_table_sub + 5) ^ 2
vis_dn$edges$width <- vis_dn$edges$weight ^ 1.5

net_res <- res_deg[[i]][res_deg[[i]]$Gene %in% vis_dn$nodes$id, ]
...
for (j in seq_along(title)) {
  net_data_sub <- net_data_dn[net_data_dn$symbol == vis_dn$nodes$id[j], ]
  title[j] <- paste(net_data_sub$gene_set, collapse = "<br>")
}
...
rm(
  ...
  res_gse, sig_deg, sig_gse, top_anno, top_deg, top_gse, vis_dn, vis_up
)
gc()
```
