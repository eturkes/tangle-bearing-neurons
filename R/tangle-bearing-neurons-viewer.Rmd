...
click_219_unscaled <- function(df, output) {
  if (is.null(df)) {
    df$row_label <- top_gse[[2]][[19]][1]
  }
...
click_219_scaled <- function(df, output) {
  if (is.null(df)) {
    df$row_label <- top_gse[[2]][[19]][1]
  }
...
shinyApp(
  ...
  server = function (input, output, session) {
    observeEvent({
      input[[paste0("gse", 219)]]
      input[[paste0("scaling", 219)]]
      input[[paste0("init", 219)]]
    }, {

      sig <- sig_gse[[2]][
        rownames(sig_gse[[2]]) %in% input[[paste0("gse", 219)]], , drop = FALSE
      ]
      if (nrow(sig) > 1) {
        sig <- sig[
          match(input[[paste0("gse", 219)]], rownames(sig)), , drop = FALSE
        ]
      }
      ...
      if (input[[paste0("scaling", 219)]] == "Scaled") {
        mat <- t(
          apply(
            mat, MARGIN = 1,
            FUN = function (x) ((2 * (x - min(x)) / (max(x) - min(x))) - 1)
      ...
      ht <- draw(
        Heatmap(
          ...
          column_title = paste0(
            data_sets[2], "\n",
            "Significant ", names(priority_gse)[19], " Pathways ",
            "(Adj. P < 0.05)"
          ),
          cluster_columns = FALSE,
          cluster_rows = FALSE,
      ...
      if (input[[paste0("scaling", 219)]] == "Scaled") {
        makeInteractiveComplexHeatmap(
          input, output, session, ht, res = 80,
          heatmap_id = paste0("ht", 219), click_action = click_219_scaled
        )
      } else {
        makeInteractiveComplexHeatmap(
          input, output, session, ht, res = 80,
          heatmap_id = paste0("ht", 219), click_action = click_219_unscaled
        )
      }
    })
  },
...
click_318_unscaled <- function(df, output) {
  if (is.null(df)) {
...
click_319_unscaled <- function(df, output) {
  if (is.null(df)) {
    df$row_label <- top_gse[[3]][[19]][1]
  }
...
click_319_scaled <- function(df, output) {
  if (is.null(df)) {
    df$row_label <- top_gse[[3]][[19]][1]
  }
...
shinyApp(
  ...
  server = function (input, output, session) {
    observeEvent({
      input[[paste0("gse", 319)]]
      input[[paste0("scaling", 319)]]
      input[[paste0("init", 319)]]
    }, {

      sig <- sig_gse[[3]][
        rownames(sig_gse[[3]]) %in% input[[paste0("gse", 319)]], , drop = FALSE
      ]
      if (nrow(sig) > 1) {
        sig <- sig[
          match(input[[paste0("gse", 319)]], rownames(sig)), , drop = FALSE
        ]
      }
      ...
      if (input[[paste0("scaling", 319)]] == "Scaled") {
        mat <- t(
          apply(
            mat, MARGIN = 1,
            FUN = function (x) ((2 * (x - min(x)) / (max(x) - min(x))) - 1)
      ...
      if (input[[paste0("scaling", 319)]] == "Scaled") {
        makeInteractiveComplexHeatmap(
          input, output, session, ht, res = 80,
          heatmap_id = paste0("ht", 319), click_action = click_319_scaled
        )
      } else {
        makeInteractiveComplexHeatmap(
          input, output, session, ht, res = 80,
          heatmap_id = paste0("ht", 219), click_action = click_319_unscaled
        )
      }
    })
  },
      ...
      output$heatmap2c <- renderPlot({
        draw(
          Heatmap(
            ...
            show_column_names = FALSE,
            cell_fun = function(j, i, x, y, width, height, fill) {
              grid.text(sig2c[i, j], x = x, y = y, gp = gpar(fontsize = 11))
            },
            heatmap_legend_param = list(
      ...
      output$heatmap3c <- renderPlot({
        draw(
          Heatmap(
            ...
            show_column_names = FALSE,
            cell_fun = function(j, i, x, y, width, height, fill) {
              grid.text(sig3c[i, j], x = x, y = y, gp = gpar(fontsize = 11))
            },
            heatmap_legend_param = list(
...
shinyApp(
  ...
  ui = fluidPage(
    fluidRow(
      column(
        width = 2,
        radioButtons(
          paste0("scaling", "_degall"), label = NULL,
          c("Scaled", "Unscaled"), inline = TRUE
        )
      )
    ),
    fluidPage(
      div(
        style = "display:flex;",
        plotOutput("heatmap2ca", height = "30000px"),
        plotOutput("heatmap3ca", height = "30000px")
      )
    )
  ),
  ...
  server = function (input, output, session) {
    observeEvent({
      input[[paste0("scaling", "_degall")]]
    }, {

      features2ca <- unique(c(top_deg[[2]], top_deg[[3]]))
      sig2ca <- sig_deg[[2]][
        rownames(sig_deg[[2]]) %in% unique(c(top_deg[[2]], top_deg[[3]])), ,
        drop = FALSE
      ]
      features2ca <- features2ca[features2ca %in% rownames(sig2ca)]
      if (nrow(sig2ca) > 1) {
        sig2ca <- sig2ca[match(features2ca, rownames(sig2ca)), , drop = FALSE]
      }

      gene_anno_sub2ca <- gene_anno[gene_anno$symbol %in% features2ca, ]
      gene_anno_sub2ca <- gene_anno_sub2ca[
        match(features2ca, gene_anno_sub2ca$symbol),
      ]

      mat2ca <- deg[[2]][
        rownames(deg[[2]]) %in% gene_anno_sub2ca$ensembl, , drop = FALSE
      ]
      if (nrow(mat2ca) > 1) {
        mat2ca <- mat2ca[
          match(gene_anno_sub2ca$ensembl, rownames(mat2ca)), , drop = FALSE
        ]
      }
      rownames(mat2ca) <- gene_anno_sub2ca$symbol

      gene_anno_sub2ca <- gene_anno[gene_anno$ensembl %in% rownames(deg[[2]]), ]
      add_anno2ca <- unique(c(top_deg[[2]], top_deg[[3]]))[
        unique(c(top_deg[[2]], top_deg[[3]])) %notin% gene_anno_sub2ca$symbol
      ]
      add2ca <- matrix(NA, nrow = length(add_anno2ca), ncol = ncol(deg[[2]]))
      rownames(add2ca) <- add_anno2ca
      mat2ca <- rbind(mat2ca, add2ca)
      sig2ca <- rbind(sig2ca, add2ca)
      sig2ca[is.na(sig2ca)] <- ""
      mat2ca <- mat2ca[
        match(unique(c(top_deg[[2]], top_deg[[3]])), rownames(mat2ca)),
      ]
      sig2ca <- sig2ca[
        match(unique(c(top_deg[[2]], top_deg[[3]])), rownames(sig2ca)),
      ]

      if (input[[paste0("scaling", "_degall")]] == "Scaled") {
        mat2ca <- t(
          apply(
            mat2ca,
            MARGIN = 1,
        ...
        col2ca <- colorRamp2(c(-1, 0, 1), colors = c("blue", "white", "red"))
        title2ca <- paste0("Scaled ", data_types[2])
      } else {
        col2ca <- colorRamp2(range_deg[[2]], colors = c("blue", "white", "red"))
        title2ca <- data_types[2]
      }

      output$heatmap2ca <- renderPlot({
        draw(
          Heatmap(
            mat2ca,
            col = col2ca,
            ...
            show_column_names = FALSE,
            cell_fun = function(j, i, x, y, width, height, fill) {
              grid.text(sig2ca[i, j], x = x, y = y, gp = gpar(fontsize = 11))
            },
            heatmap_legend_param = list(
              title = title2ca, direction = "horizontal",
              title_position = "topcenter", labels_gp = gpar(fontsize = 10),
              grid_height = unit(3, "mm"), title_gp = gpar(fontsize = 12)
            )
          ),
      ...
      features3ca <- unique(c(top_deg[[2]], top_deg[[3]]))
      sig3ca <- sig_deg[[3]][
        rownames(sig_deg[[3]]) %in% unique(c(top_deg[[2]], top_deg[[3]])), ,
        drop = FALSE
      ]
      features3ca <- features3ca[features3ca %in% rownames(sig3ca)]
      if (nrow(sig3ca) > 1) {
        sig3ca <- sig3ca[match(features3ca, rownames(sig3ca)), , drop = FALSE]
      }

      gene_anno_sub3ca <- gene_anno[
        gene_anno$symbol %in% features3ca,
      ]
      gene_anno_sub3ca <- gene_anno_sub3ca[
        match(features3ca, gene_anno_sub3ca$symbol),
      ]

      mat3ca <- deg[[3]][
        rownames(deg[[3]]) %in% gene_anno_sub3ca$ensembl, , drop = FALSE
      ]
      if (nrow(mat3ca) > 1) {
        mat3ca <- mat3ca[
          match(gene_anno_sub3ca$ensembl, rownames(mat3ca)), , drop = FALSE
        ]
      }
      rownames(mat3ca) <- gene_anno_sub3ca$symbol

      gene_anno_sub3ca <- gene_anno[
        gene_anno$symbol %in% unique(c(top_deg[[2]], top_deg[[3]])),
      ]
      gene_anno_sub3ca <- gene_anno_sub3ca[
        match(unique(c(top_deg[[2]], top_deg[[3]])), gene_anno_sub3ca$symbol),
      ]

      gene_anno_sub3ca <- gene_anno[gene_anno$ensembl %in% rownames(deg[[3]]), ]
      add_anno3ca <- unique(c(top_deg[[2]], top_deg[[3]]))[
        unique(c(top_deg[[2]], top_deg[[3]])) %notin% gene_anno_sub3ca$symbol
      ]
      add3ca <- matrix(NA, nrow = length(add_anno3ca), ncol = ncol(deg[[3]]))
      rownames(add3ca) <- add_anno3ca
      mat3ca <- rbind(mat3ca, add3ca)
      sig3ca <- rbind(sig3ca, add3ca)
      sig3ca[is.na(sig3ca)] <- ""
      mat3ca <- mat3ca[
        match(unique(c(top_deg[[2]], top_deg[[3]])), rownames(mat3ca)),
      ]
      sig3ca <- sig3ca[
        match(unique(c(top_deg[[2]], top_deg[[3]])), rownames(sig3ca)),
      ]
      ...
      if (input[[paste0("scaling", "_degall")]] == "Scaled") {
        ...
        col3ca <- colorRamp2(c(-1, 0, 1), colors = c("blue", "white", "red"))
        title3ca <- paste0("Scaled ", data_types[3])
      } else {
        col3ca <- colorRamp2(range_deg[[3]], colors = c("blue", "white", "red"))
        title3ca <- data_types[3]
      }

      output$heatmap3ca <- renderPlot({
        draw(
          Heatmap(
            mat3ca,
            col = col3ca,
            ...
            show_column_names = FALSE,
            cell_fun = function(j, i, x, y, width, height, fill) {
              grid.text(sig3ca[i, j], x = x, y = y, gp = gpar(fontsize = 11))
            },
            heatmap_legend_param = list(
              title = title3ca, direction = "horizontal",
              title_position = "topcenter", labels_gp = gpar(fontsize = 10),
              grid_height = unit(3, "mm"), title_gp = gpar(fontsize = 12)
            )
          ),
...
rm(
  ...
  gene_anno_sub2c, gene_anno_sub3c, mat2ca, mat3ca, sig2ca, sig3ca, add2ca,
  add3ca, add_anno2ca, add_anno3ca, gene_anno_sub2ca, gene_anno_sub3ca
)
gc()
```
